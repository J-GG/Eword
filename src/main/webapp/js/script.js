$(document).ready(function() {
    ...
    //Remove the success/error icon for the password field as there is already the 'unmask" icon
    ...
    $("#signup_form input").keyup(function(target)
    {
        $(this).siblings("[data-validator='perso']").remove();
        $(this).parent().siblings("[data-validator='perso']").remove();
    });
    ...
    updateImage("#user_account_picture_form");
                ...
                fields: {
                    picture: {
                        onSuccess: function(e, data) {
                            ...
                            $(idForm).find("small").hide();

                            //The file is retrieved before being transmitted
                            formData = new FormData();
                            file = $(idForm).find('.img_uploader')[0].files[0];
                            formData.append('user_picture', file);
                            $(idForm).find(".img_overlay_loading").show();
                            ...
                            $.post({
                                ...
                                {
                                    $(idForm).find(".img_overlay_loading").hide();

                                    //The error message is shown if there is any
                                    if (accountData.error !== "null") {
                                        $(idForm).children(".form-group").addClass("has-error").removeClass("has-success");
                                        $(idForm).find("small[data-validator=perso]").text(accountData.error).show();
                                    } else
                                    {
                                        //The picture is replaced by the new one
                                        var src = $(idForm).find("img").prop("src");
                                        $(idForm).find("img").prop("src", src.replace(src.substring(src.indexOf("images/pictures/") + 16), accountData.picture + "?" + new Date().getTime()));
                                    }
                                }
                            });
                        },
                        onError: function(e, data) {
                            ...
                            $(idForm).find("small[data-validator=perso]").hide();
                        }
                    }
                }
            });
